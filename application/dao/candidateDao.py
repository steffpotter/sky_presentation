...
class CandidateDao(BaseDao):
    ...
    def getAll(self): # Will cause an issue if multiple candidates have the same name
    ...
    def getCandidateById(self, candidate_id):  # Will cause an issue if multiple candidates have the same name
        ...
        return Candidate(candidate.get("candidate_id"), candidate.get("first_name"), candidate.get("last_name"),
